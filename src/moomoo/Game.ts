...
import {
    PacketType
} from "../packets/PacketType";
import FileAsync from "lowdb/adapters/FileAsync";
import {
    PacketFactory
} from "../packets/PacketFactory";
...
let MAX_CPS =
    (process.env.MAX_CPS && parseInt(process.env.MAX_CPS, 10)) || DEFAULT_MAX_CPS;
...
export default class Game {
    ...
    async addClient(id: string, socket: WebSocket, ip: string) {
        ...
        if (this.clients.length > 6969)
            this.kickClient(client, "Maximum Server Limit Reached");
                        ...
                        for (let hitGameObject of hitGameObjects) {
                            if (hitGameObject.health !== -1) {
                                ...
                                hitGameObject.health -=
                                    getStructureDamage(player.selectedWeapon) * dmgMult;
    ...
    onMsg(client: Client, packet: Packet) {
        ...
        switch (packet.type) {
            case PacketType.SPAWN:
                ...
                if (
                    ...
                    if (!player || (player && player.dead)) {
                        ...
                        newPlayer.name =
                            packet.data[0].name > 1e2 || packet.data[0].name === "" ?
                            "unknown" :
                            packet.data[0].name;
                        ...
                        newPlayer.food = Infinity;
                        newPlayer.points = Infinity;
                        newPlayer.stone = Infinity;
                        newPlayer.wood = Infinity;
            ...
            case PacketType.CHAT:
                ...
                if (
                    packet.data[0].startsWith("/") &&
                    (client.admin || packet.data[0].startsWith("/login "))
                ) {
                    console.runCommand(
...
/*                const isAccessory: boolean = !!packet.data[2];
                ...
                const isOwned: boolean = isAccessory ? client.ownedAccs.includes(packet.data[1]) : client.ownedHats.includes(packet.data[1]);

                if (client.player) {

                    if (!shouldPurchase) {
                        if (!isAccessory) {
                            if (isOwned || equipPrice <= 0 || equipID <= 0) {
                                if (client.player.hatID === equipID) {
                                    client.player.hatID = 0;
                                    client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [0, 0, 0])));
                                } else {
                                    if (client.player.points >= equipPrice && !client.ownedHats.includes(equipID)) {
                                        client.player.hatID = equipID;
                                        client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [0, equipID, 0])));
                                    }
                                }
                            }
                        } else {
                            if (isOwned || equipPrice <= 0) {
                                if (client.player.accID === equipID) {
                                    client.player.accID = 0;
                                    client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [0, 0, 1])));
                                } else {
                                    if (client.player.points >= equipPrice && !client.ownedAccs.includes(equipID)) {
                                        client.player.accID = equipID;
                                        client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [0, equipID, 1])));
                                    }
                                }
                            }
                        }
                        ...
                        if (!isAccessory) {
                            if (!client.ownedHats.includes(equipID) && client.player.points >= equipPrice) {
                                client.player.points = Math.max(0, client.player.points - equipPrice);
                                client.ownedHats.push(equipID);
                                client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [1, equipID, 0])));
                            }
                        } else {
                            if (client.ownedAccs.includes(equipID) && client.player.points >= equipPrice) {
                                client.player.points = Math.max(0, client.player.points - equipPrice);
                              console.log('e')
                                client.ownedAccs.push(equipID);
                                client.socket.send(packetFactory.serializePacket(new Packet(PacketType.UPDATE_STORE, [1, equipID, 1])));
                            }
                        }
                    }
                }*/
                        ...
                        if (packet.data[0]) {
                            if (isHat) {
                              if (!client.ownedHats.includes(packet.data[1])) {
                                const hatPrice: number = getHat(packet.data[1])?.price || 0;
                                if (client.player.points >= hatPrice) {
                                  client.player.points -= hatPrice;
                            ...
                            if (isAcc) {
                                if (!client.ownedAccs.includes(packet.data[1])) {
                                    const accPrice: number = getAcc(packet.data[1])?.price || 0;
                                    if (client.player.points >= accPrice) {
                            ...
                            if (isHat) {
                                const isOwnedHat: boolean = client.ownedHats.includes(
                                    packet.data[1]
                                );
                                const hatPrice: number = getHat(packet.data[1])?.price || 0;
                                if (isOwnedHat || hatPrice === 0 || packet.data[1] === 0) {
                                ...
                                const isOwnedAcc: boolean = client.ownedAccs.includes(
                                    packet.data[1]
                                );
                                const accPrice: number = getAcc(packet.data[1])?.price || 0;
                                if (isOwnedAcc || accPrice === 0 || packet.data[1] === 0) { // - comment
            ...
            case PacketType.CLAN_KICK:
                if (client.player) {
                    ...
                    if (tribeIndex < 0) return;
                    if (!tribe?.membersSIDs.includes(packet.data[0])) return;
            ...
            case PacketType.SELECT_UPGRADE:
                ...
                if (client.player) {
                    ...
                    let upgrades = getUpgrades(client.player.upgradeAge, client.admin);
                    let weaponUpgrades = getWeaponUpgrades(
                    ...
                    if (item <= 15) {
                        ...
                        if (upgrades.includes(item)) {
                            ...
                            if (preItem) {
                                if (!client.player.items.includes(item - preItem)) return;
                            }
